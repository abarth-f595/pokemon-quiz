...
export const baseQuizData = {
  japanese: {
    ...
    imageUrl: "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/10238.png",
...
// 生成問題を各教科にマージ
const idMap = { japanese: '国語', math: '算数', science: '理科', society: '社会', english: '外国語' };
generatedSubjects.forEach(genSubj => {
  const target = baseQuizData[Object.keys(idMap).find(k => idMap[k] === genSubj.id)];
  if (target) {
    const existingIds = new Set(target.questions.map(q => q.id));
    target.questions = [...target.questions, ...genSubj.questions.filter(q => !existingIds.has(q.id))];
  }
});
...
// 新規問題ファイルをマージ
const mergeQuestions = (subjectKey, questions) => {
  const existing = new Set(quizData[subjectKey].questions.map(q => q.id));
  quizData[subjectKey].questions = [
    ...quizData[subjectKey].questions,
    ...questions.filter(q => !existing.has(q.id))
  ];
};
mergeQuestions('english', englishQuestions);
